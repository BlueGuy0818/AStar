...
{
    int count = 0;
    while (utf8vector_next_unichar(vector, utf8) != '\0')
        count ++;
...
wchar_t utf8vector_next_unichar_with_raw(utf8vector vector, string& utf8, const char **raw, int *raw_length)
{
    ...
    else if ( (*utf & 0xE0) == 0xC0 )
    {
		...
		utf8.push_back(utf[0]);
		utf8.push_back(utf[1]);
        return uni;
    }
    //三个字节
...
{
    return utf8vector_next_unichar_with_raw(vector, utf8, NULL, NULL);
}
